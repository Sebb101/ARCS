...
#define SpeedTest_plus 7
#define SpeedTest_minus 8
#define stop_roll 9

#define test_interval_ms 1000.0f
int hertz = 1;

uint8_t RollplusState = 0;
uint8_t RollminusState = 0;
uint8_t PitchplusState = 0;
uint8_t PitchminusState = 0;
uint8_t YawplusState = 0;
uint8_t YawminusState = 0;

uint32_t test_solenoid_cmd = 99;
...
#define control_interval_ms 120.0f

uint32_t control_timer = 100;

// Connectivity //
#define SAMPLE_TIME_USB_MS 200.0f

// Message indicators
#define MSG_START '<'
#define MSG_END '>'

// Manual Solenoid Actuation
#define COMMAND_MANU_ROLL_PLUS 'a'
#define COMMAND_MANU_ROLL_MINUS 'd'
#define COMMAND_MANU_PITCH_PLUS 'w'
#define COMMAND_MANU_PITCH_MINUS 's'
#define COMMAND_MANU_YAW_PLUS 'q'
#define COMMAND_MANU_YAW_MINUS 'e'
#define COMMAND_MANU_ALL_OFF 'o'

// Control Loop
#define COMMAND_INIT_CONTROL_LOOP 'c'
#define COMMAND_STOP_CONTROL_LOOP 'x'

// Data
#define COMMAND_DATA_REQUEST 'r'
#define COMMAND_DATA_STREAM 'f'

uint8_t msg_buffer[64];

uint8_t loopState = 0;
uint8_t data_send_State = 0;
...
void solenoidTest(uint32_t TestState,int tick,int timer){
	// Test Sequence
	switch(TestState){

	  case Roll_plus_test:

		  if( (tick - timer) >= control_interval_ms){
			  GPIOB->ODR = ~(~(GPIOB->ODR) | 1U<<9); //Set PB9 as LOW
		  } else {
			  GPIOB->ODR |= 1U<<9; //Set PB9 as HIGH
		  }
...
//    }
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
		...
		// Command
		uint8_t command = msg_buffer[0];

		switch(command){
			case COMMAND_MANU_ROLL_PLUS:
				test_solenoid_cmd = Roll_plus_test;
				Solenoid_timer = HAL_GetTick();
				msg_buffer[0] = ' ';
				break;

			case COMMAND_MANU_ROLL_MINUS:
				test_solenoid_cmd = Roll_minus_test;
				Solenoid_timer = HAL_GetTick();
				msg_buffer[0] = ' ';
				break;

			case COMMAND_MANU_PITCH_PLUS:
				test_solenoid_cmd = Pitch_plus_test;
				Solenoid_timer = HAL_GetTick();
				msg_buffer[0] = ' ';
				break;

			case COMMAND_MANU_PITCH_MINUS:
				test_solenoid_cmd = Pitch_minus_test;
				Solenoid_timer = HAL_GetTick();
				msg_buffer[0] = ' ';
				break;

			case COMMAND_MANU_YAW_PLUS:
				test_solenoid_cmd = Yaw_plus_test;
				Solenoid_timer = HAL_GetTick();
				msg_buffer[0] = ' ';
				break;

			case COMMAND_MANU_YAW_MINUS:
				test_solenoid_cmd = Yaw_minus_test;
				Solenoid_timer = HAL_GetTick();
				msg_buffer[0] = ' ';
				break;

			case COMMAND_MANU_ALL_OFF:
				GPIOB->ODR = ~(~(GPIOB->ODR) | 1U<<9); //Set PB9 as LOW
				GPIOB->ODR = ~(~(GPIOB->ODR) | 1U<<8); //Set PB8 as LOW
				GPIOB->ODR = ~(~(GPIOB->ODR) | 1U<<5); //Set PB5 as LOW
				GPIOB->ODR = ~(~(GPIOB->ODR) | 1U<<4); //Set PB4 as LOW
			...
			case COMMAND_INIT_CONTROL_LOOP:
				loopState = 1;
				control_timer = HAL_GetTick();
				msg_buffer[0] = ' ';
				break;

			case COMMAND_STOP_CONTROL_LOOP:
				loopState = 0;
				control_timer = HAL_GetTick();
				msg_buffer[0] = ' ';
				break;
		...
		// Manual Solenoid Exe
		solenoidTest(test_solenoid_cmd,HAL_GetTick(),Solenoid_timer);
	  ...
	  if( ((HAL_GetTick() - Data_Timer) >= DATA_RATE_MS)){
		  readAll(&hi2c1, &MPU9255);
		  Data_Timer = HAL_GetTick();
	  }

//	RollplusState = (GPIOB->IDR & (1U<<9));
//	RollminusState = (GPIOB->IDR & (1U<<8));
//	PitchplusState = (GPIOB->IDR & (1U<<5));
//	PitchminusState = (GPIOB->IDR & (1U<<4));
//	YawplusState = (GPIOB->IDR & (1U<<3));
//	YawminusState = (GPIOA->IDR & (1U<<15));


	  // Send Data through USB, according to sample time
	  if( ((HAL_GetTick() - USB_Timer) >= SAMPLE_TIME_USB_MS) && data_send_State ){
		  float timeB = HAL_GetTick();

		  sprintf(logBuf,"%0.3f,%0.3f,%0.3f,%0.3f,%0.3f,%0.3f,%0.3f\r\n",(timeB/1000),ax,ay,az,gx,gy,gz);
	  ...
	  if( ((HAL_GetTick() - control_timer) >= control_interval_ms) && (loopState)){
		  ...
		  // Yaw Plus Angular Rate
		  if(gy > 6) {
			  if(YawminusState){
				  GPIOA->ODR = ~(~(GPIOA->ODR) | 1U<<15); //Set PA15 as LOW
				  YawminusState = 0;
			  } else {
				  GPIOA->ODR |= 1U<<15; //Set PB3 as HIGH
				  YawminusState = 1;
			  }
		  } else {
			  GPIOA->ODR = ~(~(GPIOA->ODR) | 1U<<15); //Set PA15 as LOW
